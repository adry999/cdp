-- Renaming and unpublishing a project in the same save created a redirect to
-- a slug that's no longer live — the old URL would redirect straight into a
-- 404 instead of just 404ing directly. A redirect should only exist while
-- there's something published to redirect *to*.
--
-- Also closes a cross-column collision: slug_ro and slug_en were each unique
-- on their own, but nothing stopped project A's slug_ro from equaling
-- project B's slug_en. server/api/projects/[slug].get.ts resolves a slug by
-- matching either column with maybeSingle() — a collision would make that
-- query match two rows and 500.
...
begin
  ...
  if v_id is null then
    ...
    -- Only create a redirect while the project stays published — a redirect
    -- to a now-unpublished (or newly-unpublished) slug just chains into a 404.
    ...
    -- The project just got unpublished (regardless of a rename) — any
    -- redirect still pointing at it as a destination now dead-ends, so
    -- remove those too rather than leaving a redirect into a 404.
