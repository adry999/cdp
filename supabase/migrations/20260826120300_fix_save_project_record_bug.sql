-- save_project (20260826120200) used a `record` variable for the previous
-- project row, populated only on the update path. On a *new* project, that
-- record is never assigned — and PL/pgSQL raises "record is not assigned yet"
-- the moment any field on it is referenced anywhere in an expression, even
-- inside a CASE branch that never runs. Caught by testing the RPC directly
-- before wiring the editor to it; never reached production traffic.
--
-- Fix: three scalar variables instead of one record. A scalar defaults to
-- NULL when unassigned, which is exactly the value the new-project path needs.
...
begin
  ...
  if v_id is null then
    ...
    if v_prev_published_at is not null then
