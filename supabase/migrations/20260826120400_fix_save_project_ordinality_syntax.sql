-- `jsonb_to_recordset(...) AS f(col type, ...) WITH ORDINALITY` is not valid
-- Postgres syntax — WITH ORDINALITY cannot attach to a function call that
-- carries its own column-definition list. It must wrap a ROWS FROM(...)
-- instead. Caught by testing the RPC directly, same session as the previous
-- fix; never reached production traffic.
