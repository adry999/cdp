-- shared/utils/projectPayload.ts validates required RO fields before
-- publishing — but that's a browser-side check. The RPC itself only
-- validated auth and slug shape, so any authenticated client calling
-- save_project directly (a stale tab, a script, a future bug in the editor)
-- could publish a project missing its title, summary, lead or context.
-- Mirrors the client validator's text-field invariants — NOT its gallery
-- image requirement: all 3 currently-published projects have zero gallery
-- images (see TODO.md — none exist yet), so that particular client-side rule
-- was already blocking the admin from re-saving any of them. Removed there
-- too in this same change; never added here.
